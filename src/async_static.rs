...
pub struct AsyncStatic<R: AsyncRawOnce, T> {
    fused: AsyncFused<R, Thunk<T, ConstBoxFuture<T>>>,
}

impl<R: Send + Sync + AsyncRawOnce, T: 'static + Sync + Send> AsyncStatic<R, T> where R::GuardMarker: Send {
...
unsafe impl<R: Send + Sync + AsyncRawOnce, T: Send + Sync> Sync for AsyncStatic<R, T> {}
