...
impl<R: AsyncRawOnce, T: 'static + Send> AsyncOnce<R, T> {
    ...
    pub async fn get_or_init_fn<Fu>(&self, f: impl FnOnce() -> Fu) -> &Fu::Output where Fu: 'static + Send + Future<Output=T> {
        let occupied = match self.lock().await {
            AsyncOnceEntry::Vacant(x) => {
                x.start(f())
            }
    ...
    pub async fn get_or_init<'a, Fu>(&'a self, f: Fu) -> &'a Fu::Output where Fu: 'static + Send + Future<Output=T> {
